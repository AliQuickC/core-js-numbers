...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vector1Module = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vector2Module = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalarMultiplication / (vector1Module * vector2Module));
  // throw new Error('Not implemented');
}
